package com.east.east_utils.ui.recycleview.gallery;
...
public class GalleryLayoutManager extends RecyclerView.LayoutManager implements RecyclerView.SmoothScroller.ScrollVectorProvider {
    ...
    private float MILLISECONDS_PER_INCH = 25f;//控制滑动速度
    ...
    private boolean mCanScroll = true;//recycleView是否可以滑动
    ...
    public GalleryLayoutManager(int orientation,boolean canScroll) {
    ...
    public void onLayoutChildren(RecyclerView.Recycler recycler, RecyclerView.State state) {
        if (BuildConfig.DEBUG) {
            LogUtils.d(TAG, "onLayoutChildren() called with: state = [" + state + "]");
        }
        if (getItemCount() == 0) {
            reset();
        ...
        if (state.getItemCount() != 0 && !state.didStructureChange()) {
            if (BuildConfig.DEBUG) {
                LogUtils.d(TAG, "onLayoutChildren: ignore extra layout step");
            }
            return;
        }
...
//        mCurSelectedPosition = -1;
    ...
    private void firstFillCover(RecyclerView.Recycler recycler, RecyclerView.State state, int scrollDelta) {
        ...
        if (BuildConfig.DEBUG) {
            LogUtils.d(TAG, "firstFillCover finish:first: " + mFirstVisiblePosition + ",last:" + mLastVisiblePos);
        }
    ...
    private float calculateToCenterFraction(View child, float pendingOffset) {
        ...
        if (BuildConfig.DEBUG) {
            LogUtils.d(TAG, "calculateToCenterFraction: distance:" + distance + ",childLength:" + childLength);
        }
    ...
    private void fillWithVertical(RecyclerView.Recycler recycler, RecyclerView.State state, int dy) {
        if (BuildConfig.DEBUG) {
            LogUtils.d(TAG, "fillWithVertical: dy:" + dy);
        }
        int topEdge = getOrientationHelper().getStartAfterPadding();
        int bottomEdge = getOrientationHelper().getEndAfterPadding();
            ...
            if (dy >= 0) {
                ...
                for (int i = 0; i < getChildCount(); i++) {
                    ...
                    if (getDecoratedBottom(child) - dy < topEdge) {
                        if (BuildConfig.DEBUG) {
                            LogUtils.i(TAG, "fillWithVertical: removeAndRecycleView:" + getPosition(child) + ",bottom:" + getDecoratedBottom(child));
                        }
                        ...
                        if (BuildConfig.DEBUG) {
                            LogUtils.d(TAG, "fillWithVertical: break:" + getPosition(child) + ",bottom:" + getDecoratedBottom(child));
                        }
                        break;
                    }
                }
                ...
                for (int i = getChildCount() - 1; i >= 0; i--) {
                    ...
                    if (getDecoratedTop(child) - dy > bottomEdge) {
                        if (BuildConfig.DEBUG) {
                            LogUtils.i(TAG, "fillWithVertical: removeAndRecycleView:" + getPosition(child));
                        }
                        removeAndRecycleView(child, recycler);
                        mLastVisiblePos--;
                    } else {
        ...
        if (dy >= 0) {
            ...
            for (int i = startPosition; i < getItemCount() && startOffset < bottomEdge + dy; i++) {
                ...
                mLastVisiblePos = i;
                if (BuildConfig.DEBUG) {
                    LogUtils.d(TAG, "fillWithVertical: add view:" + i + ",startOffset:" + startOffset + ",mLastVisiblePos:" + mLastVisiblePos + ",bottomEdge" + bottomEdge);
                }
            }
        } else {
            //dy<0
    ...
    private void fillWithHorizontal(RecyclerView.Recycler recycler, RecyclerView.State state, int dx) {
        ...
        int rightEdge = getOrientationHelper().getEndAfterPadding();
        if (BuildConfig.DEBUG) {
            LogUtils.i(TAG, "fillWithHorizontal() called with: dx = [" + dx + "],leftEdge:" + leftEdge + ",rightEdge:" + rightEdge);
        }
        //1.remove and recycle the view that disappear in screen
        View child;
        if (getChildCount() > 0) {
            if (dx >= 0) {
                ...
                for (int i = 0; i < getChildCount(); i++) {
                    ...
                    if (getDecoratedRight(child) - dx < leftEdge) {
                        ...
                        fixIndex--;
                        if (BuildConfig.DEBUG) {
                            LogUtils.i(TAG, "fillWithHorizontal:removeAndRecycleView:" + getPosition(child) + " mFirstVisiblePosition change to:" + mFirstVisiblePosition);
                        }
                    } else {
                        break;
                    }
                ...
                for (int i = getChildCount() - 1; i >= 0; i--) {
                    ...
                    if (getDecoratedLeft(child) - dx > rightEdge) {
                        ...
                        mLastVisiblePos--;
                        if (BuildConfig.DEBUG) {
                            LogUtils.i(TAG, "fillWithHorizontal:removeAndRecycleView:" + getPosition(child) + "mLastVisiblePos change to:" + mLastVisiblePos);
                        }
                    }
                }
            }
        ...
        if (dx >= 0) {
            if (getChildCount() != 0) {
                ...
                if (BuildConfig.DEBUG) {
                    LogUtils.d(TAG, "fillWithHorizontal:to right startPosition:" + startPosition + ",startOffset:" + startOffset + ",rightEdge:" + rightEdge);
                }
            }
            for (int i = startPosition; i < getItemCount() && startOffset < rightEdge + dx; i++) {
                ...
                mLastVisiblePos = i;
                if (BuildConfig.DEBUG) {
                    LogUtils.d(TAG, "fillWithHorizontal,layout:mLastVisiblePos: " + mLastVisiblePos);
                }
            }
        } else {
            //dx<0
            if (getChildCount() > 0) {
                ...
                if (BuildConfig.DEBUG) {
                    LogUtils.d(TAG, "fillWithHorizontal:to left startPosition:" + startPosition + ",startOffset:" + startOffset + ",leftEdge:" + leftEdge + ",child count:" + getChildCount());
                }
            }
    ...
    public boolean canScrollHorizontally() {
        return mCanScroll && mOrientation == HORIZONTAL;
    }


    @Override
    public boolean canScrollVertically() {
        return mCanScroll && mOrientation == VERTICAL;
    }
    ...
    public int scrollHorizontallyBy(int dx, RecyclerView.Recycler recycler, RecyclerView.State state) {
        ...
        if (BuildConfig.DEBUG) {
            LogUtils.d(TAG, "scrollHorizontallyBy: dx:" + dx + ",fixed:" + delta);
        }
    ...
    public int scrollVerticallyBy(int dy, RecyclerView.Recycler recycler, RecyclerView.State state) {
        ...
        if (BuildConfig.DEBUG) {
            LogUtils.d(TAG, "scrollVerticallyBy: dy:" + dy + ",fixed:" + delta);
        }
        getState().mScrollDelta = -delta;
        fillCover(recycler, state, -delta);
            ...
            if (snap != null) {
                ...
                if (selectedPosition != mCurSelectedPosition) {
                    ...
                    if (!mCallbackInFling && mState != RecyclerView.SCROLL_STATE_IDLE) {
                        if (BuildConfig.DEBUG) {
                            LogUtils.i(TAG, "ignore selection change callback when fling ");
                        }
        ...
        public void onScrollStateChanged(RecyclerView recyclerView, int newState) {
            ...
            if (mState == RecyclerView.SCROLL_STATE_IDLE) {
                ...
                if (snap != null) {
                    ...
                    LogUtils.e(TAG, "onScrollStateChanged: snap null");
                }
            }
        }
    }
    ...
    private class GallerySmoothScroller extends LinearSmoothScroller {
        ...
        @Override
        protected float calculateSpeedPerPixel(DisplayMetrics displayetrics) {
            return MILLISECONDS_PER_INCH / displayetrics.density;
        }
    ...
    public void setSpeedSlow() {
        ...
        MILLISECONDS_PER_INCH = DisplayUtil.getScreenDensity(mRecyclerView.getContext()) * 0.3f;
    }
    ...
    public void setSpeedFast() {
        MILLISECONDS_PER_INCH = DisplayUtil.getScreenDensity(mRecyclerView.getContext()) * 0.03f;
    }
}
