...
public final class EncryptUtils {
    ...
    public final static String DES_TRANSFORMATION_CBC = "DES/CBC/PKCS5Padding";//DES是加密方式 CBC是工作模式 PKCS5Padding是填充模式（秘钥一定得是8个字节的倍数）
    public final static String DES_TRANSFORMATION_ECB = "DES/ECB/NOPadding";//DES是加密方式 ECB是工作模式 NOPadding是填充模式(不填充，那么就需要加密数据为8个字节的倍数，秘钥也得是8个字节的倍数)
    public final static String DES_IVPARAMETERSPEC = "01020304";////初始化向量参数，AES 为16bytes. DES 为8bytes.
    private  static final String AES_CBC_PKCS5_PADDING = "AES/CBC/PKCS5Padding";//AES是加密方式 CBC是工作模式 PKCS5Padding是填充模式
    ...
    /**
     * 如果是DES的 ecb 模式加密就需要填充为8位
     * @param keyRule 用户指定的key
     */
     ...
     * @param data           需要加密的数据
     * @param key            密码
     * @param algorithm      加密算法的名字
     * @param transformation 加密的模式
     * @param iv             偏移量
     * @param isEncrypt      true 进行加密
     * @return the bytes of symmetric encryption or decryption
     */
